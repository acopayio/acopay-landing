/**
 * Same-origin proxy: browser → /api/pay/confirm → VPS (auto Telegram notify after Phantom sign).
 */
type PagesContext = {
  request: Request;
  env: {
...
const DEFAULT_UPSTREAM = "http://vmi3457424.contaboserver.net/pay/confirm";
...
function resolveUpstream(env: PagesContext["env"]): string {
  ...
  if (sponsor) return sponsor.replace(/\/pay\/sponsor\/?$/, "/pay/confirm");
  ...
  try {
    ...
    return new Response(text, {
      ...
      headers: {
        "Content-Type": "application/json; charset=utf-8",
        "Cache-Control": "no-store",
        "X-Acopay-Pay": "confirm-proxy",
      },
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    return json(502, { error: `Confirm upstream unreachable: ${msg}` });
  }
}
